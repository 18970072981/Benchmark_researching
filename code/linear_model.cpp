...
int main(){
    // Compare the result of the serial and parallel version
    ...
    std::cout << "The size of the serial segments is " << serial_segments.size() << std::endl;

    for(int i = 0;i<=10;i++){
        std::cout << "The slope is " << serial_segments[i].first << " and the intercept is " << serial_segments[i].second << std::endl;
    }

    start = std::chrono::high_resolution_clock::now();
    num_segments = pgm::internal::make_segmentation_par(data.size(), epsilon, in, out_parallel);
    ...
    std::cout << "The size of the parallel segments is " << parallel_segments.size() << std::endl;
    return 0;
